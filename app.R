...
ui <- dashboardPage(title="Salesa",
  ...
  dashboardSidebar(
     tags$head(tags$link(rel="shorctut icon", href="/favicon.ico")),
    sidebarMenu(
              ...
              fluidRow(
                ...
                column(2,
                       box(width=NULL, status='warning', title = "Options", solidHeader = TRUE,
                           ...
                           hr(),
                           checkboxInput("invSalesVelCheckbox", 'Compute sales velocity', value = FALSE),
                           selectInput("invVelocityPeriodSelect", "Choose past period for velocity", choices = c("Year-to-date", "30 days", "60 days", "90 days", "120 days", "180 days", "360 days")),
                           #selectInput("invForecastPeriodSelect", "Choose period for forecasting", choices = c("60 and 90 days", "30 days", "60 days", "90 days", "120 days", "180 days", "360 days")),
                           checkboxGroupInput("invForecastPeriodCheckbox", 
                                              "Choose periods for forecasting:",
                                              c("30 days" = "30",
                                                "60 days" = "60",
                                                "90 days" = "90",
                                                "120 days"= "120"), 
                                              selected = c("60 days", "90 days")),
                           submitButton(" Go "), 
                           hr(),
                           p(class='text-muted',
                             'Before downloading, press \'Go\' first or else wrong results will be downloaded'),
...
server <- function(input, output, session) {
  ...
  output$lowInventoryDataTable <- renderDataTable({
    ...
    columnNames <- c('Routing #', 'SKU', 'Brand', 'On Hands',
                     'Sales Order', 'Back Order', 'Production Order', 
                     'Available')
    
    # Compute sales velocity for the past 180 days
    if (input$invSalesVelCheckbox) {
      daysBack <- switch(input$invVelocityPeriodSelect,
                         "Year-to-date" = yday(Sys.Date()),
                         "30 days" = 30,
                         "60 days" = 60,
                         "90 days" = 90,
                         "120 days" = 120,
      ...
      columnNames <- c(columnNames, "Units Sold")
      ...
      # browser()
      InventoryForecast <- function(inventory, daysAhead) {
        for (oneForecast in daysAhead) {
          expectedSales <- ceiling(inventory$UnitsSoldPerDay * oneForecast)
          forecastColName <- paste0("stockAfter_", oneForecast, "days")
          inventory[[forecastColName]] <- inventory$Remaining + inventory$OnPurchaseOrder - expectedSales 
        }
        return(inventory)
      }
      # Add columns with forecasts
      forecastPeriods <- input$invForecastPeriodCheckbox
      if (!is.null(forecastPeriods)) {
        inventory <- InventoryForecast(inventory, as.numeric(forecastPeriods))
        # Add name of sales velocity column to the list of datatable column titles
        # and predicted stock column name
        remainingStockColNames <- paste0("Stock after ", forecastPeriods, " days")
        columnNames <- c(columnNames, remainingStockColNames)
      }
      #browser()
    } 
    

    toShow <- inventory[, -which(names(inventory) %in% c("Style", "Vendor", "UnitsSoldPerDay", "ExpectedSales"))]
